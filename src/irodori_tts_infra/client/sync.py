...
class SyncIrodoriClient:
    ...
    def synthesize_stream(self, request: SynthesisRequest) -> Iterator[bytes]:
        health = self.health()
        response = self._request("POST", "/synthesize_stream", json=_json_body(request))
        yield from _iter_stream_payloads(response.content, health.max_chunk_size)
...
def _iter_stream_payloads(data: bytes, health_max_chunk_size: int) -> Iterator[bytes]:
    offset = 0
    expected_index = 0
    ...
    while offset < len(data):
        ...
        header_line = data[offset : newline_index + 1]
        ...
        payload_end = offset + header.byte_length
