...
| Split/merge | Replace ad hoc JSON dicts with pydantic models. `HealthResponse` replaces the prototype's plain `ok` text and MUST include `max_chunk_size: int` so clients can discover the server-advertised cap before streaming. **v4 stream framing spec (wire version 1, locked for Phase 1):** JSON-line header terminated by a single `\n` byte, followed by exactly `nbytes` bytes of WAV payload (for chunk headers only). All integers are JSON numbers (portable, no endianness concerns). Parsers MUST route the two disjoint header models by `kind`: **`StreamHandshakeHeader`** `{"kind":"handshake","v":1,"max_chunk_size":N}` carries NO payload, MUST appear exactly once as the first frame of every connection, and MUST NOT use `index` / `nbytes` / `final`; **`StreamChunkHeader`** `{"kind":"chunk","v":1,"index":I,"nbytes":L,"final":B,"elapsed":S,"error_code":E?}` carries exactly `nbytes` payload bytes immediately after its line terminator. `elapsed` is always serialized in seconds. `error_code` is omitted from normal wire frames; a terminal error frame MUST set `final: true` and `nbytes: 0`, and clients map its code to a typed client error. `index` is monotonic per request, range `[0, 2^32)`, and starts at 0 for the first chunk; the required handshake does NOT shift or consume an `index` slot. `nbytes` range is `[0, max_chunk_size]`. `max_chunk_size` defaults to 4 MiB (servers MAY lower it; clients MUST accept any positive value up to the advertised cap). The handshake `max_chunk_size` MUST NOT exceed `/health.max_chunk_size`; clients MUST use the required handshake value for that connection and close with a protocol error if it exceeds the health cap. `final: true` is mandatory on the terminal chunk and no frame may follow it. Header version bumps require explicit server/client/contract migration. Client resource guards are independent of framing validity: `max_response_bytes` limits total decoded non-stream response bytes and total successful stream payload bytes (default 64 MiB), while positive `max_stream_frames` limits all chunk frames, including empty and terminal-error frames (default 65,536). Callers using small `max_chunk_size` values MUST configure a finite `max_stream_frames` large enough for their intended payload and batch shape; the default is a DoS-safe policy, not a wire-format maximum. |
| Tests | Unit tests for model validation, JSON round-trip, default synthesis parameters, ordered batch results, and wire-version header serialization. Byte-exact reconstruction test: synthesize a mock WAV, frame it into multiple chunks, parse the framed stream, assert reassembled bytes match source byte-for-byte. Interop tests MUST feed server-side framer output into the client parser with the required `StreamHandshakeHeader` followed by `StreamChunkHeader` frames. Boundary tests cover `nbytes == 0`, `nbytes == max_chunk_size`, total payload exactly at `max_response_bytes`, and frame count exactly at configured `max_stream_frames`. Handshake tests MUST cover: required handshake accepted only as the first frame, missing/duplicate/late handshake rejected, payload indices still monotonic from 0, and handshake `max_chunk_size` above `/health` rejected. Negative tests MUST also cover malformed JSON, missing `\n`, truncated or surplus payload, oversized chunks, non-monotonic/gap/duplicate index, missing or unknown `kind`, unknown `v`, missing terminal `final: true`, frames after final, total payload overflow, and frame-count overflow. All negative cases fail fast and close the response. |
...
- `/synthesize_stream` wire version 1 preserves byte-exact WAV bytes and chunk order. Every response starts with exactly one handshake and ends with a chunk carrying `final: true`. Server-framer ↔ client-parser tests exercise `v`, `nbytes`, `max_chunk_size`, total-payload, and configurable frame-count boundaries. The parser fails fast and closes on malformed headers, missing separators, truncated/surplus payloads, oversized chunks, non-monotonic or duplicate indices, unknown `v`, missing or unknown `kind`, missing/duplicate/late handshake, missing final, frames after final, total payload overflow, and frame-count overflow.
